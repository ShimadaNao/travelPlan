...
window.optionDone = window.select[1]; //未回答の問い合わせが入っている連想配列

window.waitings = window.waitings; //回答済みの問い合わせが入っている連想配列

window.dones = window.dones;
var selectValue = ''; //未回答・回答済み問い合わせそれぞれの表示エリア

var waiting = document.getElementsByClassName('waiting')[0];
var done = document.getElementsByClassName('done')[0]; // if(window.waitings.length == 0 && window.dones.length == 0){
