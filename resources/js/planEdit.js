...
window.editTitle = function() {
    ...
    + "<form method='post' action=''>タイトル：<input type='text' name='title' value='"
    + planTitle + "'><br />旅行開始日：<input type='date' value='" 
    + planStart + "'><br />旅行終了日：<input type='date' value='"
    + planEnd + "'><input type='hidden' name='plan_id' value='" 
    + planId + "'><br /><input type='submit' value='更新'></form>";
}
